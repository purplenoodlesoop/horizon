...
- `due` — OPTIONAL ISO date (`YYYY-MM-DD`) the task should be finished by.
  Set it whenever the user states OR implies a deadline — "today", "by
  Friday", "before the meeting", "this week" — **including a deadline said
  earlier in the same conversation**, not only in the message that asks you
  to create the todo. Resolve relative words ("today", "tomorrow") by
  calling `now` first. Never silently drop a deadline the user gave: if you
  truly can't resolve it to a date, write it verbatim in the body.
...
check each open task's `due` date (and any date in the body). Treat any
open task whose `due` date is already in the past as **overdue**: surface
it as overdue and offer to close or reschedule it — never report a
long-past dated item as simply "pending". Otherwise stale, never-closed
todos accumulate as false pending state.
