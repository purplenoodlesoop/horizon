...
class RunCentralizedPipeline extends StreamFx<PipelineResult> {
  RunCentralizedPipeline({
    ...
    bool heartbeatMode = false,
          ...
          final result = await RunAgentLlm(
          ...
          // Suppress the sentinel reply so the user isn't notified
          // of a no-op heartbeat.
          final suppressed =
              heartbeatMode && result.text?.trim() == heartbeatOkSentinel;
          final replyText = suppressed ? null : result.text;
          if (suppressed) {
            logger.debug(
              "[$_agentId] heartbeat OK — suppressing sentinel reply",
            );
          }
